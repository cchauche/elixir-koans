defmodule Structs do
  ...
  koan "Structs are defined and named after a module" do
    person = %Person{}
    assert person == %{__struct__: Person, name: nil, age: nil}
  end
  ...
  koan "Use the put_in macro to replace a nested value" do
    ...
    assert put_in(airline.plane.maker, :airbus) == %{
      __struct__: Airline,
      name: "Southwest",
      plane: %{__struct__: Plane, passengers: 0, maker: :airbus}
    }
  end
  ...
  koan "Use the put_in macro with atoms to replace a nested value in a non-struct" do
    ...
    assert put_in(airline[:plane][:maker], :cessna) == put_in(airline.plane.maker, :cessna)
  end
end
